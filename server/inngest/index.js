...
const sendNewConnectionRequestReminder = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        // Wait for 24 hours before sending a reminder
...
export const deleteStory = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("delete-story", async () => {
      ...
      return { message: "Story deleted after 24h" };
    });
  }
);
...
const sendNotificationOfUnseenMessages = inngest.createFunction(
    ...
    async ({step}) => {
        ...
        messages.map(message => {
            unseenCount[message.to_user_id._id] = (unseenCount[message.to_user_id.id] || 0) + 1;
        })
        ...
        for(const userId in unseenCount) {
            ...
            const body = `
            <div styele="font-family: Arial, sans-serif; padding: 20px;">
            <h2>Hi ${user.full_name},</h2>
            <p>You have ${unseenCount[userId]} unseen messages</p>
            <p>Click <a href="${process.env.FRONTEND_URL}/messages" styel="color: #10b981;
            ">here</a> to view them</p>
            <br/>
            <p>Thanks, <br/>PingUp - Sray Connected</p>
            </div>`;
