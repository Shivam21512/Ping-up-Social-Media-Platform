    ...
    <div className='bg-white rounded-xl shadow p-4 space-y-4 w-full max-w-2xl'>
        ...
        <div onClick={()=> navigate('/profile/' + post.user._id)} className='inline-flex items-center gap-3 cursor-pointer'>
            ...
            <div>
                ...
                <div className='flex items-center space-x--1'>@{post.user.username} • {moment(post.createdAt).fromNow()}</div>
            </div>
        ...
        <div className='grid grid-cols-2 gap-2'>
            {
                post.image_urls.map((img, index) => (
                    <img src={img} key={index} className={`w-full h-48 object-cover
                        rounded-lg ${post.image_urls.length === 1 && 'col-span-2 h-auto'}`} alt=''></img>
                ))
            }
        ...
        border-gray-300'>
            <div className='flex items-center gap-1'>
                <Heart className={`w-4 h-4 cursor-pointer ${likes.includes
                    (currentUser._id) && 'text-red-500 fill-red-500'}`}
                    onClick={handleLike}>
                </Heart>
