        ...
        rounded-md p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab) => (
              ...
              transition-colors cursor-pointer ${currentTab === tab.label ? 'bg-white font-medium text-black'
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {
            dataArray.find((item) => item.label === currentTab).value.map((user, index) => (
              <div key={user._id + '-' + index} className='w-full max-w-88 flex gap-5 p-6 bg-white
              shadow rounded-md'>
                <img src={user.profile_picture} alt='' className='rounded-full w-12
                h-12 shadow-md mx-auto'></img>
...
//               <button onClick={()=> setCurrentTab(tab.label)} key={tab.label}
//               className={`flex items-center px-3 py-1 text-sm rounded-md
//               transition-colors cursor-pointer ${currentTab === tab.label ? 'bg-white font-mediumtext-balck' 
//               :'text-gray-500 hover:text-black'}`}>
//                 <tab.icon className='w-4 h-4'/>
//                 <span className='ml-1'>{tab.label}</span>
//                 {
